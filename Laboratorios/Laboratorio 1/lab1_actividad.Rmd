...
3. Divide en facets y colorea según `Diet`.   
...
```{r}
maximos <- pollo %>% filter(Time == 21) %>% group_by(Diet) %>% summarize(Mayor_peso=max(weight)) # Consigo el máximo peso por dieta para Time == 21
pollo <- left_join(pollo, maximos, by="Diet") %>% mutate(dif=weight-Mayor_peso) # Los agrego a la base "pollo" y calculo la diferencia con weight. Aquellos que tengan diferencia 0, serán los máximos.
...
Los argumentos de la funciÃ³n deben ser:   
